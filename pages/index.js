  ...
  if(search != undefined && verify == undefined) {
    ...
    dex.map(value => {
      if(value.name == search) {
        ...
        .then(data => (
          setPele(data.data.sprites.front_default)
        ))
        }
        skn()
      }
      ...
      <div className="bg-blue-700 min-h-screen max-h-full">
        <div className="mx-auto w-[400px]">
          <div className="absolute top-[50px] h-[50px] h-[400px] w-[400px] bg-red-600 shadow-black shadow-2xl rounded-[10px]">
            <h1 className="absolute left-[150px] text-[25px] top-[8px]"><strong>Pokedéx</strong></h1>
                    ...
                    {dex.map(value => {
                      ...
                      if(search == value.name) {
                        return (
                          <div key={value.url}>
                            <div>
                              <p className="text-[20px] capitalize"><strong>{value.name}</strong></p>
                              <div>
                                <div className="flex flex-col items-center w-[180px] h-[180px] mx-auto ml-[4px]">
                                 <img src={pele} className="absolute w-[110px] h-[110px] bottom-[165px]"/>
                                </div>                                
                              </div>
                            </div>
                          </div>
              ...
              <div className="flex flex-col">
                ...
                <div className="flex">
                  <form method="" action="" className="flex mt-[4px]">
                    <button type="submit" className="ml-[30px] w-[60px] h-[60px] mt-[1px] rounded-[30px] bg-gray-700 shadow shadow-black"></button>
                    <input autoComplete="off" name="pokemon" className="text-center bg-green-400 h-[40px] w-[150px] py-[30px] px-[10px] mx-auto flex ml-[30px] shadow shadow-black rounded-[8px] outline-none " placeholder="Buscar..."/>
                  </form>
                  <div>
